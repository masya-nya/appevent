...
export const BasketOrder: FC<BasketOrderProps> = () => {
	const basketGoodsCount = useAppSelector(
		store => store.basket.basket.length
	);
	const basketGoodsAmount = useAppSelector(store => {
		return store.basket.basket.reduce(
	...
	return (
		<div className={cl['basket-order']}>
			<div className={cl['basket-order__info']}>
				Итого: {basketGoodsCount} {pluralizeWord(basketGoodsCount)} на
				сумму {basketGoodsAmount}&#8381;
			</div>
